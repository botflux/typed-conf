...
describe("env source", () => {
	describe("load all from schema", () => {
		...
		describe("implicit env prefixing", () => {
			it("should be able to prefix all the envs all the implicit envs", async () => {
				// Given
				const fakeEnvs = {
				...
				const source = envSource({
					mode: { type: "implicit", prefix: "MYAPP" },
				});
				const schema = object({
					host: string(),
					db: object({
			...
			it("should be able to use the configured separator between the prefix and the implicit env name", async () => {
				// Given
				const fakeEnvs = {
					"my-app.host": "localhost",
					"my-app.db.url": "postgres://localhost",
				};
				const source = envSource({
					mode: {
						type: "implicit",
