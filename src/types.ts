...
type OptionalInjectSources<T extends AnySourceType> = {
	[K in T as AllOptional<K["Inject"]> extends true
		? K["name"]
		: never]?: K["Inject"];
};
...
/**
 * Maps a union of SourceTypes to an object where keys are source names
...
/**
 * Checks if all sources in the union have all-optional inject opts.
 */
...
/**
 * Maps a union of SourceTypes to load options.
