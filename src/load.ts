...
function mapTypeboxValidationError(error: TLocalizedValidationError, origins?: Origin) {
...
function parseWithOrigin<T>(
  ...
  if (caught) {
    ...
    const mappedErrors = errors.map((error) => mapTypeboxValidationError(error, origins));
...
>(
  ...
  return parseWithOrigin<Schema["type"]>(
    schema.schema,
