...
class DefaultConfigLoader2<Schema extends ObjectSchemaBuilder<Record<string, any>>, Sources extends Source<string, never>[]>
  implements ConfigLoader<Schema, Sources> {
  configSchema: Schema
  sources: Sources
  ...
  async load(opts: LoadOpts<Sources>): Promise<Prettify<Schema["schema"][typeof kType]>> {
    ...
    for (const source of this.sources) {
      // @ts-expect-error
      const o = sources[source.key]
      const loaded = await source.load2(this.configSchema.schema, previouslyLoaded, o)
...
function config2<Schema extends ObjectSchemaBuilder<Record<string, any>>, Sources extends Source<string, never>[]>(configOpts: ConfigOpts<Schema, Sources>): ConfigLoader<Schema, Sources> {
  return new DefaultConfigLoader2(configOpts.schema, configOpts.sources)
}
