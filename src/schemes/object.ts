...
export type ObjectSchema<Spec extends Record<string, BaseSchema<unknown>>> =
	BaseSchema<SpecToType<Spec>>;

export type SpecToType<Spec extends Record<string, BaseSchema<unknown>>> = {
	readonly [Key in keyof Spec]: Spec[Key]["type"];
};

export function object<Spec extends Record<string, BaseSchema<unknown>>>(
	spec: Spec,
): ObjectSchema<Spec> {
	return {
