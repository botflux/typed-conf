...
describe("load config from envs", () => {
  ...
  it.skip('should be able to coerce envs', async function () {
    // Given
    const envs = envSource();
  ...
  it.skip('should be able to validate envs', async function () {
    // Given
    const envs = envSource();
    ...
    // Then
    await expect(promise).rejects.toThrow(new Error('env:PORT must be a number'))
  })
});
